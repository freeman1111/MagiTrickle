...
Этот репозиторий — неофициальный форк MagiTrickle, производная работа от оригинала автора
Vladimir Avtsenov и участников проекта. Как и оригинал, форк распространяется под лицензией
GPL-3.0-or-later (файл `LICENSE` не изменён). Форк изменён относительно оригинала начиная с 23.09.2026,
все изменения перечислены в разделе «Отличия форка от официальной версии» ниже и видны в истории git.

Чтобы пользователи форка не шли с его проблемами к авторам оригинала, форк помечен как форк везде, где
его видит пользователь:

- `README.md` начинается с уведомления о форке;
- окно «Сведения» в веб-панели первой строкой говорит, что это неофициальный форк, и ведёт в issues форка;
- в пакетах форка `Maintainer`, `URL` и `Description` указывают на форк (см. `scripts/publish-packages.sh`);
- на GitHub репозиторий помечен как форк `MagiTrickle/MagiTrickle`.

Официальные ресурсы оригинала:
...
Рядом лежит та же сборка с версией в имени файла, например
`magitrickle_0.8.3~git20260925201924.149978e-1_aarch64-3.10_kn.ipk`. Она нужна, чтобы по ссылке было видно,
какая версия ставится, или чтобы записать себе конкретную сборку. Постоянная ссылка всегда ведёт на самую
свежую, а версию установленного пакета показывает `opkg list-installed magitrickle`.
