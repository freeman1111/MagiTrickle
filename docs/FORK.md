...
По умолчанию исключается политика с названием **`noMT`**, и ничего дописывать в конфиг не нужно:
достаточно создать в Keenetic политику доступа `noMT` и добавить в неё устройства. Название сравнивается
без учёта регистра, подойдёт и системное имя вида `Policy4`. Если такой политики на роутере нет, сервис
запускается как обычно, в лог пишется одна строка `access policy from bypassPolicies not found`, и
исключать просто нечего.

Список можно изменить в конфиге, например исключить ещё одну политику или отключить исключение совсем:

```yaml
app:
  bypassPolicies:        # значение по умолчанию: [noMT]
    - noMT
    - Guests
# bypassPolicies: []     # ничего не исключать
```
...
На платформах без RCI (OpenWrt, не-Keenetic Entware) параметр ни на что не влияет. Изменения
`bypassPolicies` и `link` в конфиге применяются после `restart` сервиса (команда `reconfigure` их не
подхватывает).
